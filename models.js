...
function reserveBook(ID_Usuario, ID_Livro, Data_Emprestimo) {
    const query = 'INSERT INTO reservas(ID_Usuario, ID_Livro, Data_Reserva) VALUES (?, ?, ?)'
...
function getBorrowBooksByID(id, callback) {
    const query = 'SELECT books.name, borrow.Data_Emprestimo, borrow.Data_Devolucao FROM borrow, books WHERE borrow.ID_Usuario = ? AND borrow.ID_Livro = books.id';
    db.query(query, [id], (err, results) => {
        if (err) {
            console.error('Erro ao encontrar usuário:', err);
            callback(err, null);
